...
- [x] **Step 3: FastAPI service** ✅ 2026-07-04 — `app/api.py`: `POST /ask` (retrieve → confidence gate → generate with llama3.2 → cited answer) + `GET /healthz`. Verified: real question answered correctly (top score 0.832, exact S3 Vectors pricing, citations); off-topic question scored ~0.0 → refused without calling the LLM (3.4s vs 183s). Honest note: 183s generation = 3b model on laptop CPU — the Path A latency tradeoff (GPU ~2s, Bedrock ~3-5s → Week 4).
...
- **Week 2 / FastAPI service:** _DRAFT (Ritik: rewrite in your own words):_ The API loads the embedding model and DB client once at startup (lifespan) because loading costs seconds while a request must cost milliseconds. `POST /ask` runs retrieve → confidence gate → generate: if the best retrieval score is under 0.35 we return "I don't know" without calling the LLM — cheaper, faster, and can't hallucinate. Pydantic models define the request/response contract, which gives us free interactive docs at `/docs`.
